...
def _handle_task(task: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # ✅ community generation (B안)
    # - worker는 DB를 절대 건드리지 않는다.
    # - worker는 이미지 생성 후 S3/local(permanent)에 업로드만 수행
    # - backend(api)가 /community/job/{job_id} polling 시 DB finalize(ImgFile insert 등)를 수행
    if task == "journal_generate_community":
        ...
        community_type = (payload.get("community_type") or total_data.get("community_type") or "").strip().lower()
        ...
        # map 타입 덮어쓰기(계정당 1개) 정책이면 prefix 삭제(best-effort)
        ...
        return {
            ...
            "stored": {
                ...
                "org_file_name": stored.org_file_name,
    ...
    while not stop["flag"]:
        try:
            ...
            try:
                ...
                # ✅ 중요: finalized 플래그는 backend가 설정한다.
                # worker는 status/result만 기록한다.
                _set_job(
                    r,
                    job_id,
                    "DONE",
...
#         return {"image_base64": encoded, "journal_type": journal_type}
#
#     # community generation (✅ B안: AI + Storage only)
#     # - worker는 DB를 절대 건드리지 않는다.
#     # - worker는 이미지 생성 후 S3/local(permanent)에 업로드만 수행
#     # - backend(api)가 job polling 시 DB finalize(ImgFile insert, review available 업데이트)를 수행
#     if task == "journal_generate_community":
#         ai_payload = payload.get("ai_payload") or {}
#         total_data = payload.get("total_data") or {}
#         template_id = int(payload.get("template_id") or (total_data.get("template_id") or 0))
...
#             raise ValueError("AI returned empty image bytes")
#
#         # ✅ map 타입은 "계정당 1개" 정책이면 기존 prefix를 지우고 덮어쓰기
#         # (backend finalize는 DB만 만지므로, 객체 삭제는 worker에서 해줘야 깔끔함)
#         from backend.app.common.service.file_upload_service import (
#             save_permanent_bytes,
#             delete_prefix,
#             build_perm_prefix,
