...
async def persist_step2_from_image_bytes(db: Session, total_data: Dict[str, Any], image_bytes: bytes) -> Dict[str, Any]:
    ...
    # ✅ map: "계정당 1개" 규칙이므로 재생성 시 기존 리소스를 확실히 정리 + update_at 갱신(핵심)
    if community_type == "map":
        # 1) S3/local prefix 삭제(best-effort)
        try:
        ...
        # 2) update_at은 Community row update가 있어야 바뀜(server_onupdate)
        #    이미지 재생성만으로도 update_at이 바뀌어야 프론트가 "갱신"을 인지함
        db.execute(
            update(Community)
...
def finalize_from_worker_result(
    ...
    *,
    ...
    # ✅ backend finalize 경로에서도 map은 update_at 갱신(안전장치)
    if community_type == "map":
        db.execute(
            update(Community)
